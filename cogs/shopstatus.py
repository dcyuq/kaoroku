...
def ensure_config(guild_id):
    ...
    if key not in config:
        config[key] = {
            ...
            "buttons": [],
            "shop_vc_id": None,
            "shop_name_open": "🟢 Shop: OPEN",
            "shop_name_closed": "🔴 Shop: CLOSED",
            "shop_is_open": False,
        }
    ...
    settings.setdefault("shop_vc_id", None)
    settings.setdefault("shop_name_open", "🟢 Shop: OPEN")
    settings.setdefault("shop_name_closed", "🔴 Shop: CLOSED")
    settings.setdefault("shop_is_open", False)
...
class ShopNameModal(discord.ui.Modal):
    def __init__(self, builder, mode):
        self.mode = mode
        self.builder = builder
        title = "Open VC Name" if mode == "open" else "Closed VC Name"
        super().__init__(title=title)
        
        default_val = builder.settings.get(f"shop_name_{mode}") or f"Shop: {mode.upper()}"
    ...
    async def on_submit(self, interaction):
        await interaction.response.defer()
        self.builder.settings[f"shop_name_{self.mode}"] = self.f_name.value
        save_config()
        await self.builder.refresh()
...
class ShopSettingsView(discord.ui.View):
    ...
    @discord.ui.select(
        ...
        placeholder="Shop Status Voice/Stage Channel",
        row=0,
    )
    async def pick_shop_vc(self, interaction, select):
        await interaction.response.defer()
        self.builder.settings["shop_vc_id"] = select.values[0].id
        save_config()
        await self.builder.refresh()

    @discord.ui.button(label="Set VC Name (Open)", style=discord.ButtonStyle.success, row=1)
    async def set_open_name(self, interaction, button):
        await interaction.response.send_modal(ShopNameModal(self.builder, "open"))

    @discord.ui.button(label="Set VC Name (Closed)", style=discord.ButtonStyle.danger, row=1)
    async def set_closed_name(self, interaction, button):
        await interaction.response.send_modal(ShopNameModal(self.builder, "closed"))
...
class BuilderView(discord.ui.View):
    ...
    def status_embed(self):
        ...
        shop_vc = guild.get_channel(settings.get("shop_vc_id"))
        lines.append("")
        lines.append("**Shop Status**")
        lines.append(f"- **VC/Stage**: {shop_vc.mention if shop_vc else 'not set'}")
        lines.append(f"- **Open Name**: {settings.get('shop_name_open')}")
        lines.append(f"- **Closed Name**: {settings.get('shop_name_closed')}")
    ...
    @discord.ui.button(label="Shop Status", style=discord.ButtonStyle.secondary)
    async def open_shop_settings(self, interaction, button):
        view = ShopSettingsView(self)
        await interaction.response.send_message(
            embed=embeds.notice("configure the shop status voice channel and names."),
            view=view,
            ephemeral=True,
        )
...
class Tickets(commands.Cog):
    ...
    @commands.hybrid_command(
        name="shopstatus",
        aliases=["toggleshop"],
        description="Toggle the shop status and rename the assigned VC.",
    )
    @app_commands.default_permissions(manage_guild=True)
    @commands.guild_only()
    async def shopstatus(self, ctx, status: str = None):
        settings = get_config(ctx.guild.id)
        if not settings or not settings.get("shop_vc_id"):
            await ctx.send(embed=embeds.error(f"Shop VC is not set up. Run `{display_prefix(ctx)}ticketsetup` first."))
            return
            
        if not is_staff(ctx.author, settings):
            await ctx.send(embed=embeds.error("Only staff can change the shop status."))
            return

        shop_vc = ctx.guild.get_channel(settings["shop_vc_id"])
        if not shop_vc:
            await ctx.send(embed=embeds.error("The configured Shop VC no longer exists. Please reconfigure it in the builder."))
            return

        is_open = settings.get("shop_is_open", False)
        
        if status:
            status = status.lower()
            if status in ["open", "on", "true"]:
                new_status = True
            elif status in ["close", "closed", "off", "false"]:
                new_status = False
            else:
                await ctx.send(embed=embeds.error("Invalid status. Use 'open' or 'close'."))
                return
        else:
            new_status = not is_open
            
        new_name = settings.get("shop_name_open") if new_status else settings.get("shop_name_closed")
        
        try:
            await shop_vc.edit(name=new_name, reason=f"Shop status toggled by {ctx.author}")
            settings["shop_is_open"] = new_status
            save_config()
            state_str = "OPEN" if new_status else "CLOSED"
            await ctx.send(embed=embeds.notice(f"Shop status is now **{state_str}**. Channel renamed to `{new_name}`."))
        except discord.Forbidden:
            await ctx.send(embed=embeds.error("I don't have permission to rename that channel. Please check my role hierarchy and permissions."))
        except discord.HTTPException as exc:
            if exc.status == 429:
                await ctx.send(embed=embeds.error("Discord is rate-limiting channel renames (max 2 per 10 mins). Try again later."))
            else:
                await ctx.send(embed=embeds.error("Failed to rename the channel due to an API error."))
