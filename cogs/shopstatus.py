...
async def update_shop_vc(guild, settings, state):
    vc_id = settings.get("shop_vc_id")
    if not vc_id:
        return
    ...
    name_key = f"shop_name_{state}"
    new_name = settings.get(name_key)
    if not new_name:
        return
    ...
    try:
        await vc.edit(name=new_name, reason="Automatic shop status update")
    except (discord.Forbidden, discord.HTTPException) as e:
        log.warning("Failed to rename shop VC %s in guild %s: %s", vc_id, guild.id, e)
...
async def sync_state(guild, force=False):
    ...
    if force or state != settings.get("state"):
        await post_state(guild, settings, state)
        await update_shop_vc(guild, settings, state)
...
class ShopVCNameModal(discord.ui.Modal):
    def __init__(self, panel, state):
        self.panel = panel
        self.state = state
        super().__init__(title=f"{state.title()} VC Name")
        
        default_val = panel.settings.get(f"shop_name_{state}") or f"Shop: {state.upper()}"
    ...
    async def on_submit(self, interaction):
        self.panel.settings[f"shop_name_{self.state}"] = self.f_name.value
        save_config()
        await interaction.response.defer()
        await self.panel.refresh()


class ShopVCSettingsView(discord.ui.View):
    ...
    @discord.ui.select(
        ...
        placeholder="pick a VC or stage channel",
        row=0,
    )
    async def pick_vc(self, interaction, select):
        self.panel.settings["shop_vc_id"] = select.values[0].id
        save_config()
        await interaction.response.defer()
        await self.panel.refresh()

    @discord.ui.button(label="Open Name", style=discord.ButtonStyle.secondary, row=1)
    async def open_name(self, interaction, button):
        await interaction.response.send_modal(ShopVCNameModal(self.panel, "open"))

    @discord.ui.button(label="Closed Name", style=discord.ButtonStyle.secondary, row=1)
    async def closed_name(self, interaction, button):
        await interaction.response.send_modal(ShopVCNameModal(self.panel, "closed"))
        
    @discord.ui.button(label="Hiatus Name", style=discord.ButtonStyle.secondary, row=1)
    async def hiatus_name(self, interaction, button):
        await interaction.response.send_modal(ShopVCNameModal(self.panel, "hiatus"))
...
class SetupView(discord.ui.View):
    ...
    def status_embed(self):
        ...
        lines = [
            ...
            f"**power** : {power}",
            f"**now** : {now_label}",
            f"**text channel** : {channel.mention if channel else 'not set'}",
            f"**voice channel** : {vc.mention if vc else 'not set'}",
            f"**opens** : {settings['open_time']}    **closes** : {settings['close_time']}",
            f"**output** : {'embed' if settings.get('use_embed', True) else 'raw text'}",
            "",
            "**vc names**",
            f"- open: `{settings.get('shop_name_open')}`",
            f"- closed: `{settings.get('shop_name_closed')}`",
            f"- hiatus: `{settings.get('shop_name_hiatus')}`",
            "",
    ...
    @discord.ui.button(label="VC Settings", style=discord.ButtonStyle.secondary, row=2)
    async def vc_settings(self, interaction, button):
        view = ShopVCSettingsView(self)
        await interaction.response.send_message(
            embed=embeds.notice("configure the voice channel and names for the shop status."),
            view=view,
            ephemeral=True
        )
