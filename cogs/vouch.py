...
DEFAULT_TEMPLATE = (
    f":03dc_cake:{PAD}a warm gratuitory cup for the {{user}}\n"
    f"{PAD}\n"
    f":shortcake1:{PAD}{{feedback}}\n"
    f"{PAD}\n"
    f":strawberri:{PAD}may their endeavors be as sweet as the treats i "
    "have eaten!\n"
    f":IceCreamSundae:{PAD}vouched by: {{voucher}}\n"
    f":dndexl:{PAD}{{date}}"
)

FIELDS = ("user", "feedback", "voucher", "date", "time", "when", "count")

ALIASES = {
    "user": "user", "seller": "user", "shop": "user", "owner": "user",
    "vouched for": "user", "for": "user", "staff": "user", "them": "user",
    "feedback": "feedback", "review": "feedback", "message": "feedback",
    "comment": "feedback", "experience": "feedback", "note": "feedback",
    "voucher": "voucher", "vouched by": "voucher", "author": "voucher",
    "customer": "voucher", "buyer": "voucher", "from": "voucher",
    "date": "date", "day": "date",
    "time": "time",
    "when": "when", "posted": "when", "vouched at": "when",
    "count": "count", "total": "count", "number": "count", "vouch number": "count",
}

SAMPLE = {
    "user": "@theshop",
    "feedback": "it was a meaningful and uplifting session, she's very "
                "considerate and kind. will come back again!",
    "voucher": "@customer",
    "date": "august 22, 2026",
    "time": "10:22 pm",
    "when": "just now",
...
def vouch_embed(guild, settings, record):
...
class SetupView(discord.ui.View):
    """The deck. Everything about vouches is reachable from here."""
    ...
    def status_embed(self):
        ...
        embed.set_footer(text="channel · format · who can vouch — editable below")
        return embed
    ...
    async def fields(self, interaction, button):
        await interaction.response.send_message(
            embed=embeds.notice(
                ...
                "when the vouch goes out. an attached image is added under "
    ...
    @discord.ui.button(label="self vouching", style=discord.ButtonStyle.secondary, row=1)
...
class Vouch(commands.Cog):
    """Collect and post customer vouches."""
    ...
    @app_commands.describe(
        user="Who you are vouching for",
        feedback="What you want to say",
    ...
    async def vouch(
        ...
        *,
        ...
        # Reading an upload and posting takes longer than the three
        # seconds Discord allows before the interaction dies.
        ...
        if settings["staff_only"] and not ctx.author.guild_permissions.manage_messages:
            await embeds.send(
                ctx,
                embeds.error(
                    "only staff can post vouches here.", title="Not allowed"
                ),
            )
            return
        ...
        if user.id == ctx.author.id and not settings["allow_self"]:
            await embeds.send(
                ctx, embeds.error("you cannot vouch for yourself.")
            )
            return

        if user.bot:
            await embeds.send(ctx, embeds.error("you cannot vouch for a bot."))
            return

        text = feedback.strip()
        if not text:
            await embeds.send(ctx, embeds.error("say something in the vouch."))
            return
        ...
        embed = vouch_embed(ctx.guild, settings, record)
        file = None
        ...
        try:
            ...
            await embeds.send(
                ctx,
                embeds.error("discord turned that vouch down. check the log."),
            )
            return
        ...
        await embeds.send(
            ctx,
            embeds.notice(
                f"posted in {channel.mention}. that is vouch "
    ...
    @vouch.command(
        name="count",
        description="How many vouches someone has.",
    )
    ...
    async def vouch_count(self, ctx, user: discord.Member = None):
        ...
        if not total:
            await embeds.send(
                ctx,
                embeds.notice(f"{target.display_name} has no vouches yet."),
            )
            return
    ...
    @vouch.command(
        name="top",
        description="Who has the most vouches.",
    )
