...
# Everything a template remembers. "when" and "repeat" are deliberately left
# out: a saved timestamp is stale the moment it is written.
TEMPLATE_FIELDS = (
    "channel_id",
    "mode",
    "content",
...
def guild_templates(guild_id):
    """Read-only view. Does not create an entry for guilds with none."""
    bank = templates.get(str(guild_id))
    return bank if isinstance(bank, dict) else {}


def template_bank(guild_id):
    """Writable view. Creates the guild entry."""
...
def apply_template(draft, saved, member):
    ...
    # Repair anything the stored copy left blank or malformed.
    ...
    # Pings never ride in on a template unless this person could have set
    # them themselves.
...
def draft_problems(draft, guild, author):
    ...
    if channel is None:
        # Either nothing was picked, or a template pointed at a channel that
...
class TemplateSaveButton(discord.ui.Button):
    def __init__(self, panel):
        super().__init__(label="Save Current", style=discord.ButtonStyle.primary, row=2)
        self.panel = panel
...
class SendBuilderView(discord.ui.View):
    ...
    def sync_controls(self):
        """Push draft state back onto the widgets.

        Loading a template rewrites the draft behind the composer's back, so
        the mode dropdown and the pings button have to be re-pointed or they
        keep showing whatever was set before.
        """
        for item in self.children:
...
class Send(commands.Cog):
    """Post a message or embed as the bot, now or scheduled."""
